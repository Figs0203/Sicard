...
# Configuración (usa variables de entorno o escribe directamente)
...
def load_airlines(conn):
    ...
    # Limpiar campo iata: eliminar espacios y tomar solo los primeros 2 caracteres
    df['iata'] = df['iata'].astype(str).str.strip().str[:2]
    # Filtrar solo códigos de exactamente 2 caracteres alfabéticos mayúsculos
    df = df[df['iata'].str.match(r'^[A-Z]{2}$', na=False)]
    # También filtrar nulos
...
def load_airports(conn):
    ...
    df = pd.read_csv(url, header=None)
    df.columns = ['id', 'name', 'city', 'country', 'iata', 'icao', 'lat', 'lon', 'alt', 'timezone', 'dst', 'tz_db', 'type', 'source']
    
    # Limpiar iata (los aeropuertos también tienen códigos de 3 letras, pero el campo es CHAR(3))
    df['iata'] = df['iata'].astype(str).str.strip()
    # Solo códigos de 3 letras mayúsculas
    ...
    for _, row in df.iterrows():
        cur.execute(
            ...
            (row['iata'], row['icao'], row['name'], row['city'], row['country'], row['lat'], row['lon'], row['alt'])
        )
    conn.commit()
